...
public class BulletEnemy extends Entity{
	public float speed;
	public Animator animator;
	public boolean initialized=false;
	public EnemyShipOne holder;
    double scale_X;
    double scale_Y;
    int number=0; // if not 0, its triple bullet
    ArrayList<BulletEnemy> copy;

    public BulletEnemy(int number, int power, ArrayList<Sprite> images, float speed, EnemyShipOne holder, int width, int height) {
		...
		for (int i = PlayScreen.entities.size()-1; i>=0; i-- ){
			if ((PlayScreen.entities.get(i) instanceof Player)  ){
				if (Collision.checkCollision(this, PlayScreen.entities.get(i))){
					...
					this.health-=1;
					if (this.health<=0){
						copy=new ArrayList<BulletEnemy>(holder.bulletArray);
						copy.remove(this);
