...
public class BlackBulletItem extends Item {
	Sound sound;

	public BlackBulletItem(float x , float y, float speed, ArrayList<Sprite> images) {
		super( x ,  y,speed, images);
		this.width = 60;
		this.height = 25;
	}
	public BlackBulletItem(float x , float y, float speed, ArrayList<Sprite> images, boolean spawnRight) {
		...
		for (int i = PlayScreen.entities.size()-1; i>=0; i-- ){
			if ((PlayScreen.entities.get(i) instanceof Player)){
				if (Collision.checkCollision(this, PlayScreen.entities.get(i))){
					((Player) PlayScreen.entities.get(i)).bulletId=3;
