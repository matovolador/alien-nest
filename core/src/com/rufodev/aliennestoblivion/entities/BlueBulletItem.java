...
public class BlueBulletItem extends Item {
	Sound sound;

	public BlueBulletItem(float x , float y,float speed,ArrayList<Sprite> images) {
		super( x ,  y,speed, images);
		this.width = 60;
		this.height = 25;
	}
	public BlueBulletItem(float x , float y,float speed,ArrayList<Sprite> images, boolean spawnRight) {
		super( x ,  y,speed, images, spawnRight);
		this.width = 60;
		this.height = 25;
	}
		...
		for (int i = PlayScreen.entities.size()-1; i>=0; i-- ){
			if ((PlayScreen.entities.get(i) instanceof Player)){
				if (com.rufodev.aliennestoblivion.data.Collision.checkCollision(this, PlayScreen.entities.get(i))){
					((Player) PlayScreen.entities.get(i)).bulletId=1;
					((Player) PlayScreen.entities.get(i)).newBulletTimeLeft=7000;
